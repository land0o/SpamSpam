...
customers.forEach(customer => {
  const emailContact = customer.contacts.email;
  emailContact.map(email => {
    emailArray.push(email);
  });
});
console.log(emailArray);
// wanted to see how .find would display
// find() - iterates the array and returns the first item in the array that meets the condition or returns undefined.

customers.forEach(customer => {
  const emailContact = customer.contacts.email;
  emailContact.find(email => {
    emailArrayFind.push(email);
  });
});
console.table(emailArrayFind);
